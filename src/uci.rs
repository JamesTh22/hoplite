    ...
    pub fn mainloop(&mut self) {
        ...
        for line in stdin.lock().lines() {
            ...
            if line == "uci" {
                ...
                if let Some(d) = depth {
                    let (best, eval) = self.search.bestmove(&mut self.board, d);
                    let mv = if best.from == 0 && best.to == 0 {
                        "0000".to_string()
                    } else {
                        best.uci()
                    };
                    writeln!(out, "info score {}", format_score(eval)).unwrap();
                    ...
                    let (best, eval) = self.search.bestmove_time(&mut self.board, budget);
                    let mv = if best.from == 0 && best.to == 0 {
                        "0000".to_string()
                    } else {
                        best.uci()
...
fn format_score(score: i16) -> String {
    ...
    if score.abs() > MATE_SCORE - 100 {
        let mate_in = (MATE_SCORE as i32 - score.abs() as i32 + 1) / 2;
        if score > 0 {
            format!("mate {}", mate_in)
        } else {
            format!("mate -{}", mate_in)
