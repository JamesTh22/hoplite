...
impl Uci {
    pub fn new() -> Self {
        Self { board: Board::new_start(), search: Search::new() }
    }
    ...
    pub fn mainloop(&mut self) {
        ...
        for line in stdin.lock().lines() {
            ...
            if line=="uci" {
                ...
                writeln!(out, "option name ParamsFile type string default params.json").unwrap();
                ...
                if name.eq_ignore_ascii_case("Hash") {
                    if let Ok(mb) = val.trim().parse::<usize>() { self.search.set_hash_mb(mb); }
                } else if name.eq_ignore_ascii_case("Threads") {
                    if let Ok(n) = val.trim().parse::<usize>() { self.search.set_threads(n); }
                ...
                let depth = line.split_whitespace().nth(1).and_then(|s| s.parse::<u32>().ok()).unwrap_or(4);
                ...
                while i < toks.len() {
                    match toks[i] {
                        "depth" => { if i+1<toks.len() { depth = toks[i+1].parse::<i32>().ok(); i+=2; } else { i+=1; } }
                        "movetime" => { if i+1<toks.len() { movetime = toks[i+1].parse::<u128>().ok(); i+=2; } else { i+=1; } }
                        "wtime" => { if i+1<toks.len() { wtime = toks[i+1].parse::<u128>().ok(); i+=2; } else { i+=1; } }
                        "btime" => { if i+1<toks.len() { btime = toks[i+1].parse::<u128>().ok(); i+=2; } else { i+=1; } }
                        "winc" => { if i+1<toks.len() { winc = toks[i+1].parse::<u128>().unwrap_or(0); i+=2; } else { i+=1; } }
                        "binc" => { if i+1<toks.len() { binc = toks[i+1].parse::<u128>().unwrap_or(0); i+=2; } else { i+=1; } }
                        "movestogo" => { if i+1<toks.len() { movestogo = toks[i+1].parse::<u128>().ok(); i+=2; } else { i+=1; } }
                        _ => { i+=1; }
                    }
                }
                ...
                if let Some(d) = depth {
                    ...
                    let mv = if best.from==0 && best.to==0 { "0000".to_string() } else { best.uci() };
                    ...
                    let budget = if let Some(mt) = movetime {
                        ...
                        compute_budget(wtime.unwrap_or(1000), winc, movestogo)
                    } else {
                        compute_budget(btime.unwrap_or(1000), binc, movestogo)
                    };
                ...
                let depth = line.split_whitespace().skip_while(|t| *t!="depth").nth(1)
                    .and_then(|s| s.parse::<i32>().ok()).unwrap_or(4);
                let best = self.search.bestmove(&mut self.board, depth);
                let mv = if best.from==0 && best.to==0 { "0000".to_string() } else { best.uci() };
                writeln!(out, "bestmove {}", mv).unwrap();
    ...
    fn handle_position(&mut self, cmd:&str) {
        ...
        match toks.next() {
            ...
            Some("fen") => {
                ...
                if fen_parts.len()==6 {
                    if let Ok(b) = Board::from_fen(&fen_parts.join(" ")) { self.board = b; }
                }
    ...
    fn parse_uci_move(&self, s:&str) -> Move {
        if s.len()<4 { return Move::default(); }
        let b = s.as_bytes();
        let ff = (b[0]-b'a') as u8; let fr=(b[1]-b'1') as u8;
        let tf = (b[2]-b'a') as u8; let tr=(b[3]-b'1') as u8;
        let from = fr*8+ff; let to = tr*8+tf;
    ...
    fn play_if_legal(&mut self, mv: Move) {
        ...
        if legal.iter().any(|m| m.from==mv.from && m.to==mv.to && m.promo==mv.promo) {
...
fn compute_budget(time_ms: u128, inc_ms: u128, movestogo: Option<u128>) -> u128 {
