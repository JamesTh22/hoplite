...
#[inline]
#[allow(dead_code)]
fn pack_move16(m: Move) -> u16 { ((m.from as u16) << 6) | (m.to as u16) | ((m.promo.unwrap_or(crate::types::PieceKind::Pawn) as u16) << 12) }
#[inline]
...
impl Search {
    ...
    #[inline] fn time_up(&self) -> bool {
        if let Some(d)=self.deadline { Instant::now() >= d } else { false }
    }
...
let sc = -self.alphabeta(b, new_depth, -beta, -alpha, 1, false, history_keys);
    ...
    fn alphabeta(&mut self, b:&mut Board, depth: i32, mut alpha: i16, beta: i16, ply: usize, _in_null: bool, history_keys: &mut Vec<u64>) -> i16 {
        self.nodes += 1;
...
// Null-move pruning (safe)
        if depth >= 3 && has_non_pawn_material(b, b.stm) && !b.in_check(b.stm) {
            ...
            let saved_stm = b.stm;
            b.stm = if saved_stm==Side::White { Side::Black } else { Side::White };
...
let sc = -self.alphabeta(b, new_depth, -beta, -alpha, ply+1, false, history_keys);
            ...
            if sc >= beta {
                if !is_capture {
                    ...
                    self.history[side_idx][idx_hist] = self.history[side_idx][idx_hist].saturating_add(bonus);
        ...
        { let mut tt = self.tt.lock();  tt.store(Entry { key: b.key, depth: (depth as i8), value, bound, best: Move::default() }); }
        value
    }
    ...
    fn qsearch(&mut self, b:&mut Board, mut alpha:i16, beta:i16, history_keys:&mut Vec<u64>) -> i16 {
        ...
        for mv in moves {
            if b.piece_at(mv.to).is_some() {
                ...
                let u = b.make_move(mv);
                if b.in_check(b.stm) { checks.push(mv); }
                b.unmake_move(mv, u);
            }
        }
...
fn score_capture(b:&Board, mv: Move) -> i32 {
    ...
    let val = |k: PieceKind| -> i32 {
        match k {
            ...
            PieceKind::Rook=>500, PieceKind::Queen=>900, PieceKind::King=>10_000
        }
    };
    ...
    for i in 0..64u8 {
        if let Some(pc)=b.piece_at(i) {
            let val = match pc.kind {
                PieceKind::Pawn=>pv[0] as i32, PieceKind::Knight=>pv[1] as i32, PieceKind::Bishop=>pv[2] as i32,
                PieceKind::Rook=>pv[3] as i32, PieceKind::Queen=>pv[4] as i32, PieceKind::King=>pv[5] as i32,
            };
            s += if pc.side==Side::White { val } else { -val };
        }
    }
    if b.stm==Side::White { s as i16 } else { (-s) as i16 }
}
